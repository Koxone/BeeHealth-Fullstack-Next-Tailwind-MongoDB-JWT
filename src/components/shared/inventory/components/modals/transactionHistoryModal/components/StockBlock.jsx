...
function StockBlock({ transaction }) {
  return (
    <div>
      <div className="mb-2 flex items-center justify-between">
        ...
        >
          ...
          {transaction?.changedFields?.includes('minStock')
            ? transaction?.minStockDelta
            : transaction?.maxStockDelta}{' '}
          unidades
        </span>
      </div>
      ...
      <div className="space-y-1 text-sm text-gray-700">
        ...
        <p>
          ...
          {transaction?.changedFields?.includes('minStock')
            ? transaction?.oldMinStock > transaction?.newMinStock
              ? 'Disminución de Stock Mínimo'
              : 'Aumento de Stock Mínimo'
            : transaction?.changedFields?.includes('maxStock')
              ? transaction?.oldMaxStock > transaction?.newMaxStock
                ? 'Disminución de Stock Máximo'
                : 'Aumento de Stock Máximo'
              : 'Precio'}
        </p>
        <p>
          <span className="font-medium text-gray-800">Cantidad Anterior:</span>{' '}
          {transaction?.changedFields?.includes('minStock')
            ? transaction?.oldMinStock
            : transaction?.oldMaxStock}{' '}
          unidades
        </p>
        <p>
        ...
        <p>
          ...
          {transaction?.changedFields?.includes('minStock')
            ? transaction?.newMinStock
            : transaction?.newMaxStock}{' '}
          unidades
        </p>
      </div>
